...
def get_cookbook_list(versions=1)
  ridley = get_chef_client()
  return ridley.cookbook.all()
end

def get_cookbook(cookbook, version, path)
  ridley = get_chef_client()
  return ridley.cookbook.download(cookbook, version, path)
end
...
get "/:name/versions/:version" do
  ...
  unless File.exists?(File.join("cookbook-cache", "#{name}-#{version}.tgz"))
    Dir.mkdir(name)
